...
function ENT:StartTouch( eEnt )
	...
	if GAMEMODE.Cars:GetCarHealth( eEnt ) < GAMEMODE.Cars:GetCarMaxHealth( eEnt ) *0.75 then
		if IsValid( self:GetPlayerOwner() ) then
			if GAMEMODE.Cars:GetCarHealth( eEnt ) <= 0 then
				self:GetPlayerOwner():AddNote( "This vehicle needs an engine overhaul first!" )
			else
				self:GetPlayerOwner():AddNote( "This vehicle needs a repair kit!" )
				self:GetPlayerOwner():AddNote( "The damage to this vehicle is too great!" )
			end
		end
	...
	GAMEMODE.Cars:ResetCarHealth( eEnt )
	...
	timer.Create( ("EmitSound%p"):format(self), 1, 1, function()
